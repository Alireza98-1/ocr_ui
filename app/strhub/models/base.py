...
from app.strhub.data.utils_data import BaseTokenizer, CharsetAdapter, CTCTokenizer, Tokenizer
...
class BaseSystem(pl.LightningModule, ABC):
    ...
    def _eval_step(self, batch, validation: bool, eval_samples_dir='/home/bamdad/naft_recognition/test_results/unique_train_1') -> Optional[STEP_OUTPUT]:
        images, labels = batch
        ...
        lines = []
        for idx, (pred, prob, gt) in enumerate(zip(preds, probs, labels)):
            ...
            if pred == gt:
                ...
                lines.append(f'{pred}__{gt}__{prob_mul}\n')
                
            total += 1
            label_length += len(pred)
        with open(os.path.join(eval_samples_dir, 'test_error_logs.txt'), 'a') as f:
            f.writelines(lines)
